...
import { X, Search, Loader2, SlidersHorizontal, UserCheck, UserX, ShieldAlert, Trash2, Eye, Users2 } from "lucide-react";
import { Pagination } from "./pagination";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { GetCostumers } from "@/api/fetch-costumersFiltered";
import { PacientTableSkeleton } from "./clientes-skeleton";
...
function RoleBadge({ role }: { role: string }) {
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black bg-orange-50 text-orange-700 dark:bg-orange-900/20 dark:text-orange-400">
      {role?.replace(/_/g, " ") ?? "—"}
    </span>
  );
}
...
export function CostumerTableFilters() {
  ...
  const { data: costumers, isLoading } = useQuery({
    ...
    queryFn: () => GetCostumers({ municipality, nome, province: provincia, page }),
  });
  ...
  function handleFilters({ province, municipality }: PacientFilterSchema) {
    setSearchParams((s) => {
      province    ? s.set("province", province)       : s.delete("province");
      municipality ? s.set("municipality", municipality) : s.delete("municipality");
      nome        ? s.set("nome", nome)               : s.delete("nome");
      s.set("page", "1");
      return s;
    });
  }
  ...
  function handleDeleteFilters() {
    setSearchParams((s) => {
      s.delete("province"); s.delete("municipality");
      s.delete("nome");     s.delete("page");
      return s;
    });
  ...
  const queryKey = ["costumers", provincia, municipality, nome, page];
      ...
      <div className="relative max-w-screen-xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-gradient-to-br from-orange-500 to-orange-600 shadow-md shadow-orange-500/25">
            ...
            <div>
              ...
              <p className="text-zinc-400 text-xs sm:text-sm font-medium mt-0.5">
                Gestão e controlo de todos os clientes
              </p>
            </div>
          </div>
        </motion.div>
                ...
                <Controller name="province" control={control}
                  render={({ field: { name, onChange, value } }) => (
                    <Select name={name} value={value} onValueChange={(v) => { onChange(v); handleProvinceChange(v); }}>
                      <SelectTrigger className="h-10 rounded-xl border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-sm font-medium">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="rounded-2xl sm:rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-400 px-4 sm:px-5 py-3 flex items-center justify-between">
              <p className="text-white font-black text-sm uppercase tracking-widest">Lista de Clientes</p>
              {costumers && (
                <span className="text-orange-100 text-xs font-bold">{costumers.pagination.total} clientes</span>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-100 dark:border-zinc-800">
                    {["ID", "Cliente", "Registo", "Localização", "Estado", "Cargo", "Ações"].map((h) => (
                  ...
                  {isLoading ? (
                    ...
                    costumers.users.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-zinc-400 font-medium whitespace-nowrap">
