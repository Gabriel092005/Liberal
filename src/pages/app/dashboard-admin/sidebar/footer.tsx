...
export function AppFooter() {
  ...
  return (
    <footer className="mt-auto w-full pb-12 px-4 md:px-6">
        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-8">
          ...
          <div className="grid grid-cols-1 sm:flex sm:flex-row gap-4 w-full sm:w-auto items-center justify-center">
            ...
            <motion.button
              ...
              className="group flex items-center gap-4 bg-white text-black px-7 py-4 rounded-[1.8rem] transition-all duration-300 hover:bg-orange-500 hover:text-white outline-none"
            >
      ...
      {/* --- DIALOG DE APP EM DESENVOLVIMENTO --- */}
      <Dialog open={showDevDialog} onOpenChange={setShowDevDialog}>
        <DialogContent className="max-w-[320px] rounded-[2.5rem] bg-white dark:bg-zinc-950 p-8 border-none shadow-2xl flex flex-col items-center text-center">
          <div className="relative mb-4">
            <div className="bg-orange-100 dark:bg-orange-900/20 p-5 rounded-full">
              <Hammer size={40} className="text-orange-500 animate-bounce" />
            </div>
            <div className="absolute -top-1 -right-1 bg-red-500 rounded-full p-1 border-4 border-white dark:border-zinc-950">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-xs font-bold text-zinc-500 uppercase tracking-widest mt-2 leading-relaxed">
              Estamos a construir algo incrível para si.
            </DialogDescription>
          </DialogHeader>

          <p className="mt-4 text-[13px] font-medium text-zinc-600 dark:text-zinc-400">
            A nossa aplicação oficial está atualmente em fase de desenvolvimento para garantir a melhor experiência em Angola.
          </p>

          <button
            onClick={() => setShowDevDialog(false)}
            className="mt-8 w-full bg-zinc-900 dark:bg-white text-white dark:text-black py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-transform active:scale-95 shadow-lg shadow-black/10 dark:shadow-white/5"
          >
            Entendido
          </button>
        </DialogContent>
