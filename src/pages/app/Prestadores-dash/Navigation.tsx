 ...
 */
/** * NAVIGATION COMPONENT - DESKTOP REFINED VERSION
 */
/** * NAVIGATION COMPONENT - HIGH-END DESKTOP VERSION
 * Foco em: Profundidade, Sombreamento e Textura
 
  /** * BOTÃO DE NAVEGAÇÃO MOBILE
   */
  /** * NAVIGATION - HIGH-END DESKTOP (LOGO ANCHORED LEFT)
 * Estética: Profundidade extrema, camadas sobrepostas e texturas de vidro.
 */
export function Navigation() {
  ...
  return (
    ...
    <OrderCallSystem />
        ...
        <div className="max-w-[1440px] mx-auto h-full flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-4 pointer-events-auto min-w-[200px]">
            ...
            <div className="flex flex-col select-none">
              <span className="font-black text-[15px] uppercase tracking-[0.3em] text-zinc-900 dark:text-zinc-50 drop-shadow-sm">Liberal</span>
              <span className="text-[9px] font-black text-orange-500 tracking-widest uppercase">Angola Business</span>
            </div>
          </div>

          {/* 2. BARRA CENTRAL (FLUTUANTE) */}
          <nav className={`
            ...
            ${scrolled 
              ? "bg-white/70 dark:bg-zinc-950/70 backdrop-blur-3xl border border-white/40 dark:border-zinc-800/50 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] translate-y-2 scale-105" 
              : "bg-white/10 dark:bg-zinc-900/10 backdrop-blur-md border border-white/10 dark:border-zinc-800/10 translate-y-0 scale-100"}
          `}>
            {NAV_ITEMS.map((item) => {
          ...
          <div className="flex items-center gap-3 pointer-events-auto min-w-[300px] justify-end">
            ...
            {/* Divisor Visual com Textura */}
            <div className="h-6 w-px bg-zinc-200 dark:bg-zinc-800 mx-1" />

            {/* Grupo de Utilidades (Notificação + Chat + Theme) */}
            ...
            {/* Menu Lateral (Vitrine) */}
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" className="rounded-2xl h-11 px-4 gap-3 hover:bg-orange-500 hover:text-white group transition-all">
              ...
              <SheetContent side="right" className="w-[400px] p-0 border-l border-zinc-200/50 dark:border-zinc-800/50">
                <Vitrine />
              </SheetContent>
            </Sheet>
          </div>
      ...
      {/* 📱 MOBILE NAV - DESIGN DE ALTA DENSIDADE */}
      <nav className="md:hidden fixed bottom-6 inset-x-6 z-50 pointer-events-none">
        <div className="pointer-events-auto bg-white/95 dark:bg-zinc-950/95 backdrop-blur-3xl border border-zinc-200/50 dark:border-zinc-800/50 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[2.2rem] h-16 px-4">
          <ul className="flex w-full h-full justify-around items-center">
            ...
            <li className="relative -top-10">
              <SearchModal coords={coords} isOnline={isOnline} trigger={
                <button className="h-16 w-16 rounded-full bg-orange-500 shadow-[0_15px_30px_rgba(249,115,22,0.4)] border-[6px] border-zinc-50 dark:border-zinc-950 flex items-center justify-center">
                  <Search size={26} className="text-white" strokeWidth={3} />
                </button>
              } />
            </li>

            {/* No Mobile, as ações extras ficam dentro de um menu de folha ou ícone */}
            <li><MobileNavItem to="/prestadores-pedidos" icon={CreditCard} label="Pedidos" active={location.pathname === "/prestadores-pedidos"} /></li>
            <li><MobileNavItem to="/profile" icon={User} label="Eu" active={location.pathname === "/profile"} /></li>
